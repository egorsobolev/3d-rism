...
void eqoz(eqoz_t *eq, double *tuv, double *d, float *f)
{
	...
	#pragma omp for
	for (i = 0; i < n; i++)
		d[i] = (r[i] + eq->solv.charge[i / nr] * eq->rism.asymcr[i % nr]) / nr;
	#pragma omp single
	for (j = 0; j < natv; j++)
		fft_r2c(3, eq->grid.fft_shape, d + j * nr, (complex_double *) (t + j * nk), 1.0);
	#pragma omp for collapse(2)
	for (j = 0; j < natv; j++)
		for (i = 0; i < nk; i++) {
			l = j * nk + i;
			t[l] = (t[l] - eq->solv.charge[j] * eq->rism.asymck[i]) * eq->solv.symc[j];
		}
	/*
	...
	for (j = 0; j < natv; ++j) {
		i = nk * j;
		k = 2 * j;
		s[i] += eq->rism.huvk0[k] * eq->solv.symc[j];
		s[i + 1] += eq->rism.huvk0[k + 1] * eq->solv.symc[j];
	}
	#pragma omp for collapse(2)
	for (j = 0; j < natv; j++)
		for (i = 0; i < nk; i++) {
				l = j * nk + i;
				s[l] = s[l] / eq->solv.symc[j] + eq->solv.charge_sp[j] * eq->rism.asymhk[i];
			}
	/*
	...
	#pragma omp single
	for (j = 0; j < natv; j++)
		fft_c2r(3, eq->grid.fft_shape, (complex_double *) (s + j * nk), d + j * nr, 1.0);
	/*
	...
	for (j = 0; j < natv; j++)
		for (i = 0; i < nr; i++) {
			l = j * nr + i;
			d[l] -= eq->solv.charge_sp[j] * eq->rism.asymhr[i] + r[l] + tuv[l];
		}
}
...
void Jx(lneq_t *eq, float *x, float *r)
{
	...
	#pragma omp single
	for (j = 0; j < eq->natv; j++)
		fftf_r2c(3, eq->grid->fft_shape, r + j * nr, (complex_float *) (d + j * nk), 1.0f);
	/*
	 d(:) = d(:) / symc
	 t(:) = 0
	*/
	...
	#pragma omp single
	for (j = 0; j < eq->natv; j++)
		fftf_c2r(3, eq->grid->fft_shape, (complex_float *) (t + j * nk), d + j * nr, 1.0f);
	#pragma omp for
	for (i = 0; i < n; i++)
		r[i] = d[i] - x[i] - nr * r[i];
}
